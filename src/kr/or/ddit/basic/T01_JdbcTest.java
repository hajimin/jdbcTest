...
public class T01_JdbcTest {
	public static void main(String[] args) {
		/*
		 	...
		 	순서 : JDBC드라이버 로딩 => 해당  DB접속 => 질의 (SQL명령을 수행한다)
		 	...
		 	2. 접속하기 : 접속이 성공하면 Connection객체가 생성된다.
		 			DriverManager.gerConnection()메서드를 이용한다.
		 	
		 	3. 질의 : Statement객체 또는 PrepareedStatement객체를 이용하여 SQL문장을 실행 한다.
		 	
		 	4. 결과  : 
		 		1) SQL문이 select일경우 -> ResultSet객체가 만들어진다.
		 			ResultSet객체에는 select한 경로가 저장된다
		 		2) SQL문이 insert, update, delete일 경우 => 정수값을 반환한디.
		...
		try {
			...
			//rs.next() => ResultSet객체의 데이터를 가리키는 포인터를 다음레코드로 이동시키고, 그곳에 자료가 있으면 true, 없으면 false를 반환한다.
			while(rs.next()) {
				//컬럼의 자료를 가져오는 방법
